...
struct PortfolioDetailView: View {
  ...
  private func marketDataSummary(for ticker: TickerDraft) -> String {
    ...
    return "Price \(TickerDraft.displayDecimalText(for: ticker.currentPrice)) | "
      + "Band \(TickerDraft.displayDecimalText(for: ticker.bandPosition))"
  }
  ...
  private var calculateSection: some View {
    VStack(alignment: .leading, spacing: 12) {
      ...
      if let calculationOutput {
          ...
          VStack(alignment: .leading, spacing: 6) {
            Text(
              "Monthly contribution: $"
                + PortfolioFormDraft.displayText(for: calculationOutput.totalAmount)
            )
            .font(.headline)
        ...
        Text(
          "Uses the local proportional-split calculator seam after validating budget, weights, "
