...
7. Open **Actions > ios-deploy > Run workflow**, set `ref` to the commit SHA, release branch, or tag to upload, then run the workflow.
8. After upload, wait for App Store Connect processing, assign the build to the internal tester group, and complete TestFlight compliance prompts if Apple requests them.
9. **Launch-day-minus-1:** run a claim-vs-code parity pass across every public launch surface (storefront copy, screenshots, and launch-post drafts) and block posting if any of these are no longer true on the candidate commit: no analytics SDK shipped, no account required, free at v1.0, and the current market-data path requires a user-supplied Massive API key.
