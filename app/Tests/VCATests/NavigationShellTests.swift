...
final class NavigationShellTests: XCTestCase {
  ...
  func testPortfolioDraftRejectsInvalidValuesBeforeSave() {
    XCTAssertThrowsError(
      try PortfolioFormDraft(name: " ", monthlyBudgetText: "100", maWindow: 50).validatedValues()
    ) { error in
      XCTAssertEqual(error as? PortfolioEditorValidationError, .emptyName)
    }
    XCTAssertThrowsError(
      try PortfolioFormDraft(name: "Core", monthlyBudgetText: "0", maWindow: 50).validatedValues()
    ) { error in
      XCTAssertEqual(error as? PortfolioEditorValidationError, .invalidBudget)
    }
    XCTAssertThrowsError(
      try PortfolioFormDraft(name: "Core", monthlyBudgetText: "100", maWindow: 100)
        .validatedValues()
    ) { error in
  ...
  func testPortfolioDraftCreatesAndUpdatesModelOnlyOnApply() throws {
    let created = try PortfolioFormDraft(name: "Core", monthlyBudgetText: "1000", maWindow: 50)
      .makePortfolio()
    ...
    let editDraft = PortfolioFormDraft(
      name: "Core Updated", monthlyBudgetText: "1500", maWindow: 200)
    XCTAssertEqual(created.name, "Core")
  ...
  func testHoldingsDraftValidatesWeightsAndDuplicateSymbols() {
    let draft = HoldingsDraft(categories: [
      ...
      CategoryDraft(
        name: "Growth", weightPercentText: "30",
        tickers: [
          TickerDraft(
            symbol: "VTI", currentPrice: Decimal(250), movingAverage: Decimal(245),
            bandPosition: Decimal(string: "0.5"), sortOrder: 0)
  ...
  func testHoldingsDraftReordersCategoriesAndTickers() {
    var draft = HoldingsDraft(categories: [
      ...
      CategoryDraft(
        id: UUID(), name: "Second", weightPercentText: "50", sortOrder: 1,
        tickers: [
  ...
  func testHoldingsDraftBlocksCalculationForMissingOrInvalidMarketData() {
    let missingMarketDataDraft = HoldingsDraft(categories: [
      CategoryDraft(
        name: "Equity", weightPercentText: "100",
        tickers: [
